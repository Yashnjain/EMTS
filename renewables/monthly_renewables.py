...
if __name__ == "__main__": 
    try: 
        ...
        bu_alerts.send_mail(
                    ...
                    mail_subject ='JOB SUCCESS - EMTS_MONTHLY_FILE_AUTOMATION_RENEWABLES',
                    mail_body = 'EMTS_MONTHLY_FILE_AUTOMATION_RENEWABLES completed successfully, Attached logs',
                    attachment_location = logfile
                )
    except Exception as e:
        logging.info(f'Error occurred in EMTS_MONTHLY_FILE_AUTOMATION_RENEWABLES {e}')
        bu_alerts.bulog(process_name=JOBNAME,status='failed',log=logfile,process_owner='Pakhi',table_name=" ")
        bu_alerts.send_mail(
                            receiver_email= receiver_email,
                            mail_subject=f"JOB FAILED - EMTS_MONTHLY_FILE_AUTOMATION_RENEWABLES",
                            mail_body=f"{e}",
                            attachment_location = logfile)
